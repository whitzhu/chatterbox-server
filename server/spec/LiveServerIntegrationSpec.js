...
describe('server', function() {
  ...
  it('should respond with all messages that were previously posted', function(done) {
    ...
    request(requestParams2, function(error, response, body) {
      ...
      request('http://127.0.0.1:3000/classes/messages', function(error, response, body) {
        ...
        expect(messages[0].username).to.equal('Jono');
        expect(messages[0].message).to.equal('Do my bidding!');
        expect(messages[messages.length - 1].username).to.equal('Jono2');
        expect(messages[messages.length - 1].message).to.equal('Do my bidding2!');
        done();
      });
    });
  });
  ...
  it('should reject DELETE requests to /classes/messages', function(done) {
